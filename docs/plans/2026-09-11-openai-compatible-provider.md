...
Status: DRAFT v4, 2026-09-11. Author: Claude (mentor/overseer). Reviewer: Codex (adversarial, iterative, on 27-mac-mini). Implementer: Codex (OMSV mentee, on 27-mac-mini).
...
D5. **Errors.** `map_completion_error`: `HttpError(InvalidStatusCode(s) | InvalidStatusCodeWithMessage(s, _))` with 401/403 -> `Unauthorized`, 429 -> `ProviderError("rate limited: ...")`, else `ProviderError` (both `send` and `send_streaming` of rig's reqwest client produce these, `http_client/mod.rs:143-149, 381-387`). Session 401 retry stays Remote-only. `NotConfigured` message: "AI is not configured. Choose a connection in Settings and add its API key or model."
...
D8. **Settings state, owned by the store.** `chat_store.ts` holds `draft: AiSettingsDraft` (provider, apiKey, openaiApiKey, openaiBaseUrl, openaiModel, serverUrl) and exports `draftFromConfig(config)`, `setDraft(patch)`, `isDraftUnsaved()`, `saveDraft()`, `switchProviderAndSave(provider)`, `loadModelSuggestions()`. `AiSettings` binds its inputs to the store draft (no local signals for config fields); `AccessPrompt` calls `switchProviderAndSave("remote")`, which preserves every other field and both secrets. Rows for OpenAI: Base URL, API key (show/hide; required/optional label from `keyRequirementFor`), Model (text + `<datalist>` from `modelSuggestions` + "Load models", the connection test). Quick guide and chat setup prompt render from `settings_copy.ts` (`guideCopyFor`, `setupPromptFor`).
...
D10. **Fork release identity and updater.** `tauri.h4.conf.json` merged over `tauri.conf.json`: identifier/productName inherited, `"version": "0.5.8-h4.1"`, `bundle.targets: ["app"]`, `bundle.createUpdaterArtifacts: false`. The updater is disabled at build time: moon task `desktop:tauri-build-h4` sets `VITE_KUKU_UPDATER=off`; `app.tsx` guards its single `checkForUpdates()` call with `shouldCheckForUpdates({ prod, updater })`. **Proof chain (three deterministic links):** (1) `src/stores/__tests__/updater.test.ts` truth table for the helper; (2) `src/stores/__tests__/updater_build_binding.test.ts` reads `apps/desktop/moon.yml` and asserts the `tauri-build-h4` task's env contains `VITE_KUKU_UPDATER: "off"` and that `app.tsx` contains exactly one `checkForUpdates(` call site and it is inside `shouldCheckForUpdates`; (3) `updater.ts` exports `UPDATER_BUILD_MARKER`, defined as `import.meta.env.VITE_KUKU_UPDATER === "off" ? "kuku-updater-disabled" : "kuku-updater-enabled"`; Vite inlines the env and esbuild constant-folds the comparison, so the production bundle contains exactly one of the two literals (claim C-UPD2, validated by the mentee on the built bundle); `release_h4.sh` greps the built `Kuku.app`'s frontend assets and refuses to publish unless `kuku-updater-disabled` is present and `kuku-updater-enabled` is absent. The marker is also shown in the settings pane's version line ("0.5.8-h4.1, updates via Homebrew") so a screenshot documents it.

D11. **Signing and publishing.** Build unsigned on 27-mac-mini; sign/notarize/staple in one `gui/$UID` LaunchAgent (Adapt's script as structural reference; the Kuku copy also signs every Mach-O under `Contents/MacOS` and nested `.framework/.dylib/.bundle/.appex/.xpc/.app`); DMG create/sign/notarize/staple; publish to tag `kuku-v<version>` on `horizonthinking/homebrew-h4`; cask `Casks/kuku.rb` (source of truth in the h4 monorepo, mirrored to the tap clone). `publish_kuku_cask.sh` hashes the DMG itself and refuses a mismatching sha, refuses a dirty or diverged tap clone, syncs `git pull --ff-only`, stages `git add -A`, has `--dry-run` and a temp-repo test. **One entry point:** Wave 4 runs `scripts/h4/release_h4.sh` and nothing else by hand; the script owns every precondition and step (section 6, Wave 4).
...
- `src/stores/__tests__/updater.test.ts` (new): T-T17 `shouldCheckForUpdates` truth table.
- `src/stores/__tests__/updater_build_binding.test.ts` (new): T-T18 moon task env binding and single guarded call site (D10).
...
### Wave 2, frontend
Files: `types.ts`, `config.ts`, `chat_store.ts` (draft ownership per D8), `provider_readiness.ts`, `chat_panel.tsx`, `components/ai_settings.tsx`, `components/model_label.ts`, `components/settings_copy.ts`, `index.ts`, `stores/updater.ts` (`shouldCheckForUpdates`, `UPDATER_BUILD_MARKER`), `app.tsx`, `env.d.ts`, `i18n/keys.ts` + three locales, tests T-T1..T-T18. Acceptance: `desktop:test-ts`, `desktop:lint-ts-check`, `desktop:format-ts-check`, `desktop:build` green.
...
## 10. OMSV protocol
Overseer/mentor (Claude) as in v3. Mentee (Codex on 27-mac-mini via `codex_exec_guard.sh`, `workspace-write` with network): kb-app worktree `~/projects/apps/kb-app-worktrees/openai-provider`; for Wave 3 additionally `--add-dir ~/projects/h4-worktrees/kuku-cask`. One wave per dispatch, `WAVE_REPORT.md` with files touched, verbatim gate output, claim table; the mentee may REJECT a claim. Adversarial review loop as in v3, four rounds maximum.
...
- C-OLL1, C-OLL2, C-CFG1, C-SEC1, C-SEC2 (account `ai-chat:openaiApiKey`, service `mom.kuku.desktop.plugin-secrets`; exact `security delete-generic-password` recorded), C-CMD1 as in v3.
- C-UPD (v4): with `VITE_KUKU_UPDATER=off` from the moon task, `app.tsx`'s single guarded call never runs (T-T17, T-T18).
- C-UPD2 (new): the production bundle built by `tauri-build-h4` contains the literal `kuku-updater-disabled` and not `kuku-updater-enabled` (esbuild constant-folds the inlined env comparison); validated by grepping the built assets.
- C-BLD1, C-BLD2, C-TAP1 as in v3, with C-TAP1 now covering the SSH-URL bootstrap path.
