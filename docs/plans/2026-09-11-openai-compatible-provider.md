...
Status: DRAFT v1, 2026-09-11. Author: Claude (mentor/overseer). Reviewer: Codex (adversarial, iterative). Implementer: Codex (OMSV mentee).
...
- C-RIG4: `CompletionError::HttpError` exposes the HTTP status code (or the message contains it) well enough to classify 401/403/429 deterministically; if not, the mentee documents the actual shape and the classifier falls back to substring on the status text. (Substring is a fallback for classification only, never for a test assertion.)
...
- C-UPD: `tauri-plugin-updater` 2.x `check()` returns `Ok(None)` when the manifest `version` is not greater than the current version, before consulting `platforms`, so a manifest with the current version and `"platforms": {}` produces status `idle`, not `error`.
